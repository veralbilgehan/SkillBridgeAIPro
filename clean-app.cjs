...
// The mojibake is back because PowerShell's Add-Content mangled patch5.js.
// Let's re-apply patch5's content cleanly via Node to avoid PowerShell encoding issues.
const patch5Code = `
...
// Remove the old broken block from app.js by finding where patch5 started, or just rebuild from scratch
// Actually, since app.js is just a concatenation of index, patch, patch2, patch3, patch4, patch5...
// Let's just read it, replace mojibake globally, then find window.actualTestResults and replace the rest of the file!

const replaceMojibake = (str) => {
    const reps = {
        'Ä±': 'ı', 'Ã¼': 'ü', 'ÅŸ': 'ş', 'Åž': 'Ş', 'Ã§': 'ç', 'Ã‡': 'Ç', 'Ã–': 'Ö', 'Ã¶': 'ö', 'ÄŸ': 'ğ', 'Ä°': 'İ', 'Äž': 'Ğ', 'Ãœ': 'Ü', 'Ã¢': 'â', 'â€“': '–', 'Â·': '·'
    };
    for (const [b, g] of Object.entries(reps)) {
        str = str.split(b).join(g);
    }
    return str;
};
...
const index = c.indexOf('window.actualTestResults = [];');
if (index !== -1) {
    c = c.substring(0, index) + patch5Code + '\n})();\n';
} else {
    c += '\n(function(){\n' + patch5Code + '\n})();\n';
}

fs.writeFileSync('dist/app.js', c, 'utf8');
console.log('App.js rewritten cleanly.');
